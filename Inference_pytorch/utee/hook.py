...
def Neural_Sim(self, input, output): 
    global model_n, FP, layer_n, weight_precision, input_precision
    if FP:
        print("quantize layer ", layer_n)
        input_file_name =  './layer_record_' + str(model_n) + '/input' + str(layer_n) + '.csv'
        weight_file_name =  './layer_record_' + str(model_n) + '/weight' + str(layer_n) + '.csv'
        layer_n += 1
    else:
    ...
    if FP:
        weight_q = float_quantizer.float_range_quantize(self.weight,weight_precision)
    else:
        weight_q = wage_quantizer.Q(self.weight,weight_precision)
    write_matrix_weight( weight_q.cpu().data.numpy(),weight_file_name)
    if len(self.weight.shape) > 2:
        k=self.weight.shape[-1]
        write_matrix_activation_conv(stretch_input(input[0].cpu().data.numpy(),k),None,input_precision,input_file_name)
    else:
        write_matrix_activation_fc(input[0].cpu().data.numpy(),None ,input_precision, input_file_name)
...
def hardware_evaluation(model,wl_weight,wl_activation,model_name,mode): 
    ...
    layer_n = 0
    weight_precision = wl_weight
    input_precision = wl_activation
    ...
    for i, layer in enumerate(model.modules()):
        if isinstance(layer, (nn.Conv2d, QConv2d)) or isinstance(layer, (nn.Linear, QLinear)):
