...
parser.add_argument('--lr', type=float, default=1, help='learning rate (default: 1e-3)')
...
# FP mode uses float range quantization, different from WAGE quantization
if args.mode == 'FP':
    from utee import float_quantizer
    for name, param in list(modelCF.named_parameters())[::-1]:
        param.data = float_quantizer.float_range_quantize(param.data, args.wl_weight)
